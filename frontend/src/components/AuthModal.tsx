...
export default function AuthModal({ 
  ...
  const { loginWithEmail, signupWithEmail, loginWithGoogle, resetPasswordForEmail } = useAuth();
  ...
  // Rate limiting cooldown timer for forgot password
  ...
  const content = (
    ...
    >
      ...
      <div className="mb-6 text-center">
        ...
        >
          {tab === 'forgot' ? <KeyRound className="w-3.5 h-3.5" /> : <Sparkles className="w-3.5 h-3.5" />}
          <span>{tab === 'forgot' ? 'Account Recovery' : 'RecruitAI Platform'}</span>
        </motion.div>
        ...
        <p className="text-muted text-xs md:text-sm mt-1.5">
          {tab === 'login' && 'Sign in to access your recruitment campaigns & AI screening'}
          {tab === 'signup' && 'Get started with automated candidate intelligence'}
          {tab === 'forgot' && 'Enter your email and we will send you a secure recovery link'}
        </p>
      </div>

      {/* Smooth Spring Tab Switcher (Visible in login & signup modes) */}
      {tab !== 'forgot' ? (
      ...
      {/* Google OAuth Button (Visible in login & signup modes) */}
      {tab !== 'forgot' && (
        <>
          <motion.button
            type="button"
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              {tab === 'login' && (
                <button
                  ...
                  className="text-[11px] text-muted hover:text-accent font-medium cursor-pointer transition-colors"
                >
                  Forgot password?
                </button>
              )}
        ...
        <motion.button
          type="submit"
          disabled={loading || googleLoading || (tab === 'forgot' && cooldown > 0)}
